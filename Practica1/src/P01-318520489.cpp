...
AZUL=0.0f;
 ...
 /// se inicializa la semilla
     srand(time(nullptr));
/// se declaran variavles
//SE GUARDA EL TIEMPO INICIAL LA FUNCION GET TIME REGRESA U DOUBLE
double TempInicial= glfwGetTime();
double TempUltimCambio=TempInicial;
	//Loop mientras no se cierra la ventana
	while (!glfwWindowShouldClose(mainWindow))
	{
	double TempActual=glfwGetTime(); 

		glfwPollEvents();
		
		if(TempActual-TempUltimCambio>=2.0){
	VERDE= ColorAleatorio();
	AZUL=ColorAleatorio();
	ROJO=ColorAleatorio();
	// printf("Nuevo color: R=%.2f, G=%.2f, B=%.2f\n", ROJO, VERDE, AZUL);
	TempUltimCambio=TempActual;
	//printf("NUEVO TIEMPO : %.2f",TempUltimCambio);	
